...
class MessageController extends CommonController {
    ...
    public function send() {
        if (IS_POST) {
            ...
            if ($receiver_type == 1) { // 学员
                if ($receiver_ids) {
                    $ids = array_filter(array_map('intval', explode(',', $receiver_ids)));
                    if (!empty($ids)) {
                        $campus_where['id'] = ['in', $ids];
                    }
                }
                $receivers = M('student')->where($campus_where)->getField('id', true) ?: [];
            } elseif ($receiver_type == 2) { // 家长
                // TODO: 获取家长用户
            } elseif ($receiver_type == 3) { // 老师
                $receivers = M('teacher')->where($campus_where)->getField('id', true) ?: [];
            }

            // 批量发送
            $base_data = [
                'campus_id' => $this->tenant_campus_id > 0 ? $this->tenant_campus_id : 0,
                'title' => $title,
                'content' => $content,
                'type' => $type,
                'send_status' => 1,
        ...
        $stu_where = [];
        if ($this->tenant_campus_id > 0) {
            $stu_where['campus_id'] = $this->tenant_campus_id;
        }
        $students = M('student')->field('id,username')->where($stu_where)->limit(100)->select();
    ...
    public function log() {
        ...
        $list = M('message_log')->where($where)->order('send_time DESC')
            ->limit($offset, $limit)
            ->select();
    ...
    public function autoSendAttendance() {
        ...
        $attendances = M('attendance')->where($where)->select();
    ...
    public function hourWarning() {
        ...
        $p_where = [];
        if ($this->tenant_campus_id > 0) {
            $p_where['campus_id'] = $this->tenant_campus_id;
        }
        $p_where['status'] = 1;
        $p_where['balance_hours'] = ['elt', 5];

        $packages = M('student_package')->where($p_where)->select();
