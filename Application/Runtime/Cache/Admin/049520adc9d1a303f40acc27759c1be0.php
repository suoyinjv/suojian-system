...
<div class="content-right">
	<div class="content">
			...
			<div class="am-g">
				...
				<div class="more-where <?php if(!isset($param['is_more']) or $param['is_more'] != 1): ?>none<?php endif; ?>">
					<select name="state" class="am-radius c-p m-t-10 m-l-5 param-where">
						<option value="-1"><?php echo L('common_view_user_state_name');?></option>
						<?php if(is_array($common_user_state_list)): foreach($common_user_state_list as $key=>$v): ?><option value="<?php echo ($v["id"]); ?>" <?php if(isset($param['state']) and $param['state'] == $v['id']): ?>selected<?php endif; ?>><?php echo ($v["name"]); ?></option><?php endforeach; endif; ?>
					</select>
					...
					<div class="param-date param-where m-l-5">
						<input type="text" name="time_start" readonly="readonly" class="am-radius m-t-10" placeholder="<?php echo L('user_time_start_text');?>" id="time_start" <?php if(isset($param['time_start'])): ?>value="<?php echo ($param["time_start"]); ?>"<?php endif; ?>/>
						<span>~</span>
						<input type="text" readonly="readonly" class="am-radius m-t-10" placeholder="<?php echo L('user_time_end_text');?>" name="time_end" id="time_end" <?php if(isset($param['time_end'])): ?>value="<?php echo ($param["time_end"]); ?>"<?php endif; ?>/>
					</div>
				</div>
			</div>
        </form>
		...
		<table class="am-table am-table-striped am-table-hover am-text-middle m-t-10 m-l-5">
			<thead>
				<tr>
					<th><?php echo L('user_nickname_name');?></th>
					<th class="am-hide-sm-only"><?php echo L('common_mobile_name');?></th>
					<th class="am-hide-sm-only"><?php echo L('common_email_name');?></th>
					<th class="am-hide-sm-only"><?php echo L('common_view_gender_name');?></th>
					<th class="am-hide-sm-only"><?php echo L('common_view_user_state_name');?></th>
			...
			<tbody>
				<?php if(!empty($list)): if(is_array($list)): foreach($list as $key=>$v): ?><tr id="data-list-<?php echo ($v["id"]); ?>" <?php switch($v["state"]): case "1": ?>class="am-active"<?php break; case "2": ?>class="am-danger"<?php break; endswitch;?>>
							<td><?php if(empty($v['nickname'])): ?><span class="cr-ddd"><?php echo L('common_on_fill_in_the_text');?></span><?php else: echo ($v["nickname"]); endif; ?></td>
							<td class="am-hide-sm-only"><?php if(empty($v['mobile'])): ?><span class="cr-ddd"><?php echo L('common_on_fill_in_the_text');?></span><?php else: echo ($v["mobile"]); endif; ?></td>
							<td class="am-hide-sm-only"><?php if(empty($v['email'])): ?><span class="cr-ddd"><?php echo L('common_on_fill_in_the_text');?></span><?php else: echo ($v["email"]); endif; ?></td>
							<td class="am-hide-sm-only"><?php echo ($v["gender"]); ?></td>
							<td class="am-hide-sm-only"><?php echo ($v["state_text"]); ?></td>
							<td class="am-hide-sm-only"><?php if(empty($v['birthday'])): ?><span class="cr-ddd"><?php echo L('common_on_fill_in_the_text');?></span><?php else: echo ($v["birthday"]); endif; ?></td>
							<td>
									...
									<div class="am-popup-inner">
										...
										<div class="am-popup-bd">
											<dl class="dl-content">
												...
												<dd><?php if(empty($v['birthday'])): ?><span class="cr-ddd"><?php echo L('common_on_fill_in_the_text');?></span><?php else: echo ($v["birthday"]); endif; ?></dd>

												<dt><?php echo L('common_view_user_state_name');?></dt>
												<dd><?php echo ($v["state_text"]); ?></dd>

												<dt><?php echo L('user_signature_name');?></dt>
												<dd><?php if(empty($v['signature'])): ?><span class="cr-ddd"><?php echo L('common_on_fill_in_the_text');?></span><?php else: echo ($v["signature"]); endif; ?></dd>

												<dt><?php echo L('user_describe_name');?></dt>
												<dd><?php if(empty($v['describe'])): ?><span class="cr-ddd"><?php echo L('common_on_fill_in_the_text');?></span><?php else: echo ($v["describe"]); endif; ?></dd>
							...
							<td class="view-operation">
								<a href="<?php echo U('Admin/User/SaveInfo', array('id'=>$v['id']));?>">
								...
								<button class="am-btn am-btn-default am-btn-xs am-radius am-icon-trash-o submit-delete" data-url="<?php echo U('Admin/User/Delete');?>" data-am-popover="{content: '<?php echo L('common_operation_delete');?>', trigger: 'hover focus'}" data-id="<?php echo ($v["id"]); ?>"></button>
							</td>
