...
<div class="content-right">
	<div class="content">
		<!-- form start -->
		<form class="am-form view-list" action="<?php echo U('Admin/Teacher/Index');?>" method="POST">
			<div class="am-g">
				<input type="text" class="am-radius form-keyword" placeholder="<?php echo L('teacher_so_keyword_tips');?>" name="keyword" <?php if(isset($param['keyword'])): ?>value="<?php echo ($param["keyword"]); ?>"<?php endif; ?> />
				...
				<div class="more-where <?php if(!isset($param['is_more']) or $param['is_more'] != 1): ?>none<?php endif; ?>">
					<select name="state" class="am-radius c-p m-t-10 m-l-5 param-where">
						<option value="-1"><?php echo L('common_view_teacher_state_name');?></option>
						<?php if(is_array($common_teacher_state_list)): foreach($common_teacher_state_list as $key=>$v): ?><option value="<?php echo ($v["id"]); ?>" <?php if(isset($param['state']) and $param['state'] == $v['id']): ?>selected<?php endif; ?>><?php echo ($v["name"]); ?></option><?php endforeach; endif; ?>
					</select>
					...
					<div class="param-date param-where m-l-5">
						<input type="text" name="time_start" readonly="readonly" class="am-radius m-t-10" placeholder="<?php echo L('teacher_time_start_text');?>" id="time_start" <?php if(isset($param['time_start'])): ?>value="<?php echo ($param["time_start"]); ?>"<?php endif; ?>/>
						<span>~</span>
						<input type="text" readonly="readonly" class="am-radius m-t-10" placeholder="<?php echo L('teacher_time_end_text');?>" name="time_end" id="time_end" <?php if(isset($param['time_end'])): ?>value="<?php echo ($param["time_end"]); ?>"<?php endif; ?>/>
					</div>
				</div>
			</div>
        </form>
        ...
        <div class="am-g m-t-15">
            <a href="<?php echo U('Admin/Teacher/SaveInfo');?>" class="am-btn am-btn-secondary am-radius am-btn-xs am-icon-plus"> <?php echo L('common_operation_add');?></a>
		...
		<table class="am-table am-table-striped am-table-hover am-text-middle m-t-10">
			<thead>
				<tr>
					<th><?php echo L('teacher_username_name');?></th>
					...
					<th class="am-hide-sm-only"><?php echo L('common_view_tel_name');?></th>
					<th class="am-hide-sm-only"><?php echo L('common_view_teacher_state_name');?></th>
			...
			<tbody>
				<?php if(!empty($list)): if(is_array($list)): foreach($list as $key=>$v): ?><tr id="data-list-<?php echo ($v["id"]); ?>-<?php echo ($v["id_card"]); ?>">
								...
								<div class="am-popup am-radius" id="my-popup<?php echo ($v["id"]); ?>">
									<div class="am-popup-inner">
										...
										<div class="am-popup-bd">
											<dl class="dl-content">
												<dt><?php echo L('teacher_username_name');?></dt>
												...
												<dt><?php echo L('common_view_teacher_state_name');?></dt>
												...
												<dd><?php echo ($v["mobile"]); ?></dd>

												<dt><?php echo L('common_email_name');?></dt>
												<dd><?php echo ($v["email"]); ?></dd>

												<dt><?php echo L('teacher_birthday_text');?></dt>
												<dd><?php echo ($v["birthday"]); ?></dd>

												<dt><?php echo L('teacher_subject_text');?></dt>
												<dd><?php echo ($v["subject_list"]); ?></dd>
							...
							<td class="view-operation">
								<a href="<?php echo U('Admin/Course/SaveInfo', array('teacher_id'=>$v['id']));?>">
									<button class="am-btn am-btn-default am-btn-xs am-radius am-icon-list-alt" data-am-popover="{content: '<?php echo L('teacher_operation_course');?>', trigger: 'hover focus'}"></button>
								</a>
								<a href="<?php echo U('Admin/Teacher/SaveInfo', array('id'=>$v['id']));?>">
								...
								<button class="am-btn am-btn-default am-btn-xs am-radius am-icon-trash-o submit-delete" data-url="<?php echo U('Admin/Teacher/Delete');?>" data-am-popover="{content: '<?php echo L('common_operation_delete');?>', trigger: 'hover focus'}" data-id="<?php echo ($v["id"]); ?>-<?php echo ($v["id_card"]); ?>"></button>
							</td>
