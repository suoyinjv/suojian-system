  ...
  <div class="page">
    ...
    <el-card class="table-card" shadow="never">
      <el-table :data="list" stripe v-loading="loading">
        ...
        <el-table-column prop="name" label="星期名称" min-width="160" />
    ...
    <!-- Add Dialog -->
    <el-dialog v-model="showDialog" title="添加星期" width="480px" :close-on-click-modal="false">
      <el-form :model="form" label-width="90px" class="dialog-form">
        <el-form-item label="星期名称" required>
          <el-input v-model="form.name" placeholder="请输入星期名称，如：星期一" />
        </el-form-item>
        <el-form-item label="排序">
          <el-input-number v-model="form.sort_order" :min="0" :max="7" style="width: 180px" />
        </el-form-item>
...
const defaultForm = { name: '', sort_order: 0, status: 1 }
...
async function loadData() {
  ...
  try {
    ...
    const res = await axios.get(BASE + '/m/Admin/c/Api/a/weekList', { params })
    const data = res.data
    ...
    console.error('[weeks] error:');
    ElMessage.error('加载星期列表失败')
  } finally {
    loading.value = false
  }
}
...
async function handleSave() {
  ...
  try {
    ...
    await axios.post(BASE + '/m/Admin/c/Api/a/weekCreate', params.toString(), {
...
function handleDelete(row) {
  ElMessageBox.confirm(`确定删除「${row.name}」吗？`, '确认删除', { type: 'warning' })
    .then(async () => {
      try {
        await axios.get(BASE + '/m/Admin/c/Api/a/weekDelete?id=' + row.id)
