  ...
  <div class="page">
    ...
    <el-card class="filter-card" shadow="never">
      <el-form :model="filters" layout="inline">
        <el-form-item label="关键词">
          <el-input v-model="filters.keyword" placeholder="学生姓名/标题" clearable style="width: 240px">
    ...
    <el-card class="table-card" shadow="never">
      <el-table :data="list" stripe v-loading="loading">
        ...
        <el-table-column label="记录内容" min-width="240" prop="content" show-overflow-tooltip />
    ...
    <!-- Add Dialog -->
    <el-dialog v-model="showDialog" title="新增成长记录" width="560px" :close-on-click-modal="false">
      <el-form :model="form" label-width="90px" class="dialog-form">
        ...
        <el-form-item label="记录内容" required>
          <el-input v-model="form.content" type="textarea" :rows="5" placeholder="请输入成长记录内容" />
        </el-form-item>
      </el-form>
      <template #footer>
...
const form = reactive({ student_name: '', title: '', content: '' })
...
async function loadData() {
  ...
  try {
    ...
    const res = await axios.get(BASE + '/m/Admin/c/Api/a/growthList', { params })
    const data = res.data
...
async function handleSave() {
  if (!form.student_name || !form.title || !form.content) {
  ...
  try {
    ...
    await axios.post(BASE + '/m/Admin/c/Api/a/growthCreate', params.toString(), {
