  ...
  <div class="page">
    ...
    <el-card class="filter-card" shadow="never">
      <el-form :model="filters" layout="inline">
        ...
        <el-form-item label="关键词">
          <el-input v-model="filters.keyword" placeholder="班级名称" clearable style="width: 220px">
    ...
    <el-card class="table-card" shadow="never">
      <el-table :data="list" stripe v-loading="loading">
        ...
        <el-table-column label="上级班级" min-width="130">
    ...
    <!-- Add/Edit Dialog -->
    <el-dialog v-model="showDialog" :title="isEdit ? '编辑班级' : '添加班级'" width="440px" :close-on-click-modal="false">
      <el-form :model="form" label-width="80px" class="dialog-form">
        <el-form-item label="班级名称" required>
          <el-input v-model="form.name" placeholder="请输入班级名称" />
        </el-form-item>
        <el-form-item label="上级班级">
          <el-select v-model="form.pid" clearable style="width: 100%">
            <el-option label="无（顶级班级）" :value="0" />
...
async function loadData() {
  ...
  try {
    ...
    const res = await axios.get(BASE + '/m/Admin/c/Api/a/classList', { params })
    const data = res.data
    ...
    // Build parent options from all classes
    if (arr.length) {
...
function openEdit(row) {
  ...
  // Rebuild parent options excluding self
...
async function handleSave() {
  ...
  try {
    ...
    const url = isEdit.value
      ? BASE + '/m/Admin/c/Api/a/classUpdate'
      : BASE + '/m/Admin/c/Api/a/classCreate'
...
function handleDelete(row) {
  ElMessageBox.confirm(`确定删除班级「${row.name}」吗？`, '确认删除', { type: 'warning' })
    .then(async () => {
      try {
        await axios.get(BASE + '/m/Admin/c/Api/a/classDelete?id=' + row.id)
...
async function toggleStatus(row) {
  try {
    ...
    await axios.post(BASE + '/m/Admin/c/Api/a/classUpdate', params.toString(), {
      headers: { 'Content-Type': 'application/x-www-form-urlencoded' }
    })
